...
export class PostgresStorage implements IStorage {
  ...
  async verifySchema(): Promise<void> {
    try {
      console.log("[STORAGE] Verifying database schema...");
      // Check if system_config table exists by attempting a simple select
      await this.db.select({ key: schema.systemConfig.key }).from(schema.systemConfig).limit(1);
      console.log("[STORAGE] Database schema verified successfully.");
    } catch (error: any) {
      if (error.code === '42P01') { // undefined_table
        console.error("[STORAGE] [CRITICAL] Database tables are missing! Please run 'npm run db:push' to initialize the database.");
        throw new Error("Database not initialized. Please run 'npm run db:push'.");
      }
      console.error("[STORAGE] Database verification failed:", error.message);
      throw error;
    }
  }
